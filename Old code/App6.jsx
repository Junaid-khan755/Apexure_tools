...
const mkTestDesktop = () => ({
  ...
  cardPadding: 24,
  cardGap: 20,
  // FIX (issue 1): shadow is now controllable via toggle + sliders
  showShadow: true,
  shadowX: 0,
  shadowY: 2,
  shadowBlur: 16,
...
// ── Helper: build a CSS box-shadow string from config ─────────────────────────
function shadowCss(cfg) {
  if (!cfg.showShadow) return "none";
  const x = cfg.shadowX ?? 0;
  const y = cfg.shadowY ?? 2;
  const blur = cfg.shadowBlur ?? 16;
  const spread = cfg.shadowSpread ?? 0;
  const color = cfg.shadowColor || "rgba(0,0,0,0.08)";
...
// ── Custom Font Manager ───────────────────────────────────────────────────────
// FIX (issue 4): supports both typing a font name AND uploading a font file
// (.woff/.woff2/.ttf/.otf). Uploaded fonts are stored as base64 data URLs so
// they can be embedded directly via @font-face in both the live preview and
// the exported code — no external hosting required.
...
function CustomFontManager({ customFonts, onAdd, onAddFile, onRemove }) {
  ...
  const handleFile = (e) => {
    ...
    reader.onload = () => {
      // Derive a font-family name from the filename (without extension)
...
// FIX (issue 4): inject @font-face rules for uploaded fonts so the preview
// can actually render them.
function useUploadedFontFace(font) {
  useEffect(() => {
...
function FontLoader({ cfg, type, customFonts }) {
  ...
  // Also inject @font-face for any uploaded custom fonts in use
  fonts.forEach((f) => {
    const match = customFonts.find((c) => c.name === f && c.dataUrl);
    useUploadedFontFace(match);
  });
  return null;
}

// ── Generate Google Fonts @import + @font-face for code output ──────────────
// FIX (issue 4): include @font-face declarations for uploaded fonts so
// exported code is self-contained.
...
function genTestiCode(items, count, dCfg, tCfg, mCfg, platform, customFonts) {
  ...
  // FIX (issue 1): shadow now derived from controllable config
  const dShadow = shadowCss(dCfg);
  const tShadow = shadowCss(tCfg);
  const mShadow = shadowCss(mCfg);

  // FIX (issue 2): section background now applies to the carousel-style
  // layouts' wrapper too (it already did for card-grid; now consistent
  // across all layouts via .ub-testi-wrap)
...
.ub-testi-hscroll .ub-testi-card {
  scroll-snap-align: start;
  flex: 0 0 auto;
  width: ${Math.round(dCfg.maxWidth / (dCfg.columns || 3) - dCfg.cardGap)}px;
}
/* ── Single Quote layout ── */
.ub-testi-single {
  text-align: center;
}
.ub-testi-single .ub-testi-card {
  align-items: center;
  text-align: center;
}
.ub-testi-single .ub-testi-quote { font-size: ${Math.round(dCfg.quoteSize * 1.3)}px; }
/* ── List Feed layout ── */
.ub-testi-list {
  display: flex;
  flex-direction: column;
...
.ub-testi-list .ub-testi-card {
  flex-direction: row;
  align-items: flex-start;
  gap: ${dCfg.cardGap}px;
}
.ub-testi-list .ub-testi-card .ub-testi-body { flex: 1; display: flex; flex-direction: column; gap: 8px; }
/* ── Inline Avatar layout ── */
.ub-testi-inline-grid {
  display: grid;
  grid-template-columns: repeat(${dCfg.columns || 3},1fr);
  gap: ${dCfg.cardGap}px;
}
.ub-testi-inline-grid .ub-testi-card {
  flex-direction: row;
  align-items: flex-start;
  gap: 10px;
}
.ub-testi-inline-grid .ub-testi-card .ub-testi-body { flex: 1; display: flex; flex-direction: column; gap: 6px; }`;
...
${mC}`;
  ...
  // Shared data + stars/card-html builder used across layouts
  const dataJs = `  var testimonials = [
${vis.map((t) => `    { name:${JSON.stringify(t.name)}, role:${JSON.stringify(t.role)}, company:${JSON.stringify(t.company)}, quote:${JSON.stringify(t.quote)}, stars:${t.stars}, initials:${JSON.stringify(t.initials)}, avatarBg:${JSON.stringify(t.avatarBg)} }`).join(",\n")}
  ...
  const buildCardFn = `  function buildCard(t, layoutClass) {
    var stars = '';
    for (var s = 1; s <= 5; s++) {
      stars += '<span style="color:' + (s <= t.stars ? '${dCfg.starColor}' : '#d1d5db') + ';font-size:${dCfg.starSize}px">★</span>';
    }
    var avatarHtml = ${dCfg.showAvatar} ? '<div class="ub-testi-avatar" style="background:' + t.avatarBg + '">' + t.initials + '</div>' : '';
    var bodyHtml = '<p class="ub-testi-quote">' + t.quote + '</p>'
    ...
    var quoteMarkHtml = ${dCfg.showQuoteMark} ? '<div style="font-size:${dCfg.quoteMarkSize}px;color:${dCfg.quoteMarkColor};font-family:Georgia,serif;line-height:.8">&#8220;</div>' : '';

    if (layoutClass === 'list' || layoutClass === 'inline') {
      return '<div class="ub-testi-card">'
        + avatarHtml
        + '<div class="ub-testi-body">' + quoteMarkHtml + bodyHtml + '</div>'
        + '</div>';
    }
    return '<div class="ub-testi-card">'
      + quoteMarkHtml
  ...
  if (dCfg.layout === "card-grid") {
    js = `${isUnbounce ? "window.addEventListener('load',function(){" : 'document.addEventListener("DOMContentLoaded",function(){'}
${dataJs}
${buildCardFn}
  ...
  testimonials.forEach(function(t) {
    wrap.insertAdjacentHTML('beforeend', buildCard(t, 'grid'));
  });
});`;
...
${buildCardFn}
  ...
  track.className = 'ub-testi-hscroll';
  testimonials.forEach(function(t) {
    track.insertAdjacentHTML('beforeend', buildCard(t, 'grid'));
  });
...
${buildCardFn}
  var wrap = document.getElementById('ubTesti');
  if (!wrap) return;
  wrap.className += ' ub-testi-single';
  var current = 0;
  var slot = document.createElement('div');
  wrap.appendChild(slot);
  function render() {
    slot.innerHTML = buildCard(testimonials[current], 'single');
    ${
      dCfg.showDots
        ? `
  ...
  navRow.style.cssText = 'display:flex;justify-content:center;gap:10px;margin-top:14px;';
  var prevBtn = document.createElement('button');
  prevBtn.className = 'ub-testi-nav-btn';
  prevBtn.textContent = '${pIcon}';
  var nextBtn = document.createElement('button');
  nextBtn.className = 'ub-testi-nav-btn';
  nextBtn.textContent = '${nIcon}';
  prevBtn.addEventListener('click', function(){ current = (current - 1 + testimonials.length) % testimonials.length; render(); });
  nextBtn.addEventListener('click', function(){ current = (current + 1) % testimonials.length; render(); });
  ...
  render();
  ${dCfg.autoPlay ? `setInterval(function(){ current = (current + 1) % testimonials.length; render(); }, ${dCfg.autoPlaySpeed});` : ""}
});`;
...
${buildCardFn}
  ...
  testimonials.forEach(function(t) {
    wrap.insertAdjacentHTML('beforeend', buildCard(t, 'list'));
  });
});`;
...
${buildCardFn}
  ...
  wrap.className += ' ub-testi-inline-grid';
  testimonials.forEach(function(t) {
    wrap.insertAdjacentHTML('beforeend', buildCard(t, 'inline'));
  });
});`;
  } else {
...
${buildCardFn}
  ...
  ${
    dCfg.navStyle === "bottom"
      ? `
  var navRow2 = document.createElement('div');
  navRow2.style.cssText = 'display:flex;justify-content:center;gap:10px;margin-top:14px;';
  navRow2.appendChild(prevBtn);
  navRow2.appendChild(nextBtn);
  wrap.appendChild(navRow2);`
      : ""
  }
...
// FIX (issue 1): card now reads box-shadow from shadowCss(cfg), which
// respects the showShadow toggle + blur/spread/color sliders.
// FIX (issue 3): card now supports a "variant" for list / inline-avatar
// layouts, which render avatar + text side-by-side instead of stacked.
...
// FIX (issue 2): wrapper now always carries sectionBg, for every layout —
// not just card-grid. This is the element users see as "the carousel
// background".
...
function CarouselLayout({ items, cfg }) {
  ...
  useEffect(() => {
    ...
    if (isLast) return; // stop autoplay at last slide
    const t = setInterval(next, cfg.autoPlaySpeed || 3000);
    return () => clearInterval(t);
  }, [cfg.autoPlay, cfg.autoPlaySpeed, idx, total]);
  ...
  return (
    <div style={{ maxWidth: cfg.maxWidth, margin: "0 auto" }}>
      ...
      >
        ...
        {navSides && (
          <button onClick={next} disabled={isLast} style={navBtnStyle(isLast)}>
            {navIcon(cfg, "next")}
          </button>
        )}
      </div>
...
// FIX (issue 3): H-Scroll layout — horizontally scrollable row of cards
// with fixed-width columns based on `columns`, snap scrolling.
...
// FIX (issue 3): Single Quote layout — one large featured quote with
// prev/next + dots to step through testimonials.
function SingleQuoteLayout({ items, cfg }) {
  ...
  const bigCfg = { ...cfg, quoteSize: Math.round(cfg.quoteSize * 1.3) };
  return (
    <SectionWrap cfg={cfg}>
      <div style={{ textAlign: "center" }}>
        <div style={{ display: "inline-block", width: "100%", maxWidth: 640 }}>
          <TestiCard
            t={current}
            cfg={{ ...bigCfg, quoteAlign: "center" }}
            style={{
              alignItems: "center",
              textAlign: "center",
              margin: "0 auto",
...
// FIX (issue 3): List Feed layout — vertically stacked rows, avatar beside
// quote/name/role.
...
// FIX (issue 3): Inline Avatar layout — grid of compact cards with avatar
// inline beside the text (uses `columns`).
...
function TestiStylePanel({ cfg, onChange, customFonts }) {
  ...
  // FIX (issue 3): nav/dots/autoplay controls are now relevant to all
  // multi-item layouts that can step through items (carousel, h-scroll
  // is scroll-based so no nav needed, single-quote needs nav).
  ...
  return (
    <div style={{ padding: "12px 14px" }}>
      ...
      />
      <SHead title="Section" />
      {/* FIX (issue 2): clarified label + now visibly affects every layout's
          wrapper background, including the carousel */}
      <ColorRow
      ...
      />
      {/* FIX (issue 1): shadow toggle + custom sliders */}
      <Toggle
        label="Card shadow"
        val={cfg.showShadow}
...
export default function App() {
  ...
  // Custom fonts — FIX (issue 4): each entry is now { name, dataUrl?, format? }
  // dataUrl/format are present for uploaded font files.
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Load Google Fonts / uploaded fonts for preview */}
          {isFaq && (
